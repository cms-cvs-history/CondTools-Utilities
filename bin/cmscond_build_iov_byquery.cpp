...
int main(int argc, char** argv) {
  ...
  boost::program_options::options_description visible("Usage: cms_build_iov_byquery [options] [iov_name] \n");
  visible.add_options()
    ("connect,c",boost::program_options::value<std::string>(),"connection string(required)")
    ("query,q",boost::program_options::value<std::string>(),"query string(required)")
  ...
  if( query.empty() ){
    std::cerr<<"[Error] empty query string \n";
    return 1;
  }
  if(vm.count("user")){
